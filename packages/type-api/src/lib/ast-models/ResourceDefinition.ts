...
export class ResourceDefinition {
    ...
    constructor(
        ...
        if (!config) {
            throw new Error(`Invalid use of ${this.type}, you need to specify its config!`);
        }
        if (config.type !== 'ObjectExpression') {
            throw new Error(`${this.type} expects an object`)
        }
    ...
    public get defaultName() {
        ...
        if (!exportDeclaration) {
            throw new Error(`Could not generate name for ${this.type} as it is not assigned to any variable.`)
        }
        
        const id = exportDeclaration.get('id');
        if (id.node?.type !== 'Identifier') {
            throw new Error(`Could not locate identifier for ${this.type}!`);
        }
        return [
    ...
    public transform(targetEnvironment: TargetEnvironment) {
        if (this.transformedFor) {
            if (this.transformedFor !== targetEnvironment) {
                throw new Error(`Tried pre-transformed module to a different environment: ${targetEnvironment}`)
            }
            console.warn(new Error('Tried to transform resource twice'));
        } else {
